...
def format_image_list(directory, extension):
    files = os.listdir(directory)
    expression = re.compile('.' + extension + '\Z')
...
def load_and_resize_image(name, size, directory='.', mode = 'bw', extension = ''):
    ''' Returns a numpy array with the info about the pixels in the image
    ...
    '''
    if mode == 'color':
        arr = imresize(imread(directory + '/' + name + extension, flatten = False), size)
    elif mode == 'bw':
